...
def AHTforCircles(edge,center_threhold_factor = None,score_threhold = None,min_center_dist = None,minRad = None,maxRad = None,center_axis_scale = None,radius_scale = None,halfWindow = None,max_circle_num = None):
    center_threhold_factor = center_threhold_factor or 10.0
    score_threhold = score_threhold or 15.0
    min_center_dist = min_center_dist or 80.0
    minRad = minRad or 0.0
    maxRad = maxRad or 1e7*1.0
    center_axis_scale = center_axis_scale or 1.0
    radius_scale = radius_scale or 1.0
    halfWindow = halfWindow or 2
    max_circle_num = max_circle_num or 6
    ...
    for i in range(center_accumulator.shape[0]):
        for j in range(center_accumulator.shape[1]):
            neibor = center_accumulator[max(0, i - halfWindow + 1):min(i + halfWindow, center_accumulator.shape[0]),
